...
public class RestaurantCheckoutActivity extends AppCompatActivity {
...
//
                            FirebaseUtil.currentUserRestaurant( ).get().addOnCompleteListener (new OnCompleteListener<DocumentSnapshot> ( ) {
                                @Override
                                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                                    if (task.isSuccessful ( )) {
...
//                            completeProfile();
    ...
    private void getStoreStatus(String pin) {
        ...
        binding.textView43.setText (branch.getAnnouncement ());
        binding.policy.setText (branch.getPolicy ());
...
//                                                        googleMap.addMarker(markerOptions);
    ...
    void calculation(Double distance) {
        ...
        for (int i=0;i<products.size ();i++){
            ...
            int value= Integer.parseInt (branch.getKmcharge ());

            double delivery=value;
            radius= Integer.parseInt (branch.getRadius ());

            maintotal+=total;


            baseDeliveryCharge= Integer.parseInt (branch.getDelivery ());
            minAmount= Integer.parseInt (branch.getMinAmount ());

            del=baseDeliveryCharge+distance/1000*delivery;
        ...
        if(distance/1000>radius){
            ...
            MaterialAlertDialogBuilder alertDialog=new MaterialAlertDialogBuilder (this);
              alertDialog.setMessage ("Your location is so far from store order may be delay or cancel");
                            ...
                            new AlertDialog.Builder(RestaurantCheckoutActivity.this)
                                    ...
                                    .setPositiveButton("Track Order", new DialogInterface.OnClickListener() {
                                        ...
                                        public void onClick(DialogInterface dialogInterface, int i) {
                                            Intent intent = new Intent(RestaurantCheckoutActivity.this, DeliveryDetailActivity.class);
